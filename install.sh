...
EOF
    ...
    # Install global nodemon for development
    if ! command -v nodemon &> /dev/null; then
        echo -e "${YELLOW}Installing nodemon globally for development...${NC}"
...
test_installation() {
    ...
    cd backend
    if ! npm list express &> /dev/null; then
        echo -e "${RED}❌ Express.js not installed${NC}"
        return 1
    fi
    cd ..
...
show_final_instructions() {
    ...
    echo -e "${GREEN}npm start${NC}              # Start server (from backend/)"
    echo -e "${GREEN}npm run dev${NC}            # Development mode with auto-reload"
    echo -e "${GREEN}npm install${NC}            # Install/update dependencies"
    echo ""
    echo -e "${CYAN}🔧 Troubleshooting:${NC}"
    echo "• Check logs: backend/system.log, backend/all.log"
    echo "• Test OpenWebUI: Use 'Test OpenWebUI' button in settings"
    echo "• Test TTS: Use 'Test TTS' button in settings"
...
main() {
    ...
    echo -e "${GREEN}✅ Installation completed successfully!${NC}"
    echo -e "${CYAN}Run './start.sh' to begin!${NC}"
}
